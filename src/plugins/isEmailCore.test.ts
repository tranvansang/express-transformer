...
const repeat = (str: string, count: number) => [...Array(count).keys()].reduce(acc => acc + str, '')
const doTest = ({valid, invalid, args}: {
	valid: string[],
	invalid: string[],
	args?: IIsEmailOptions
}) => {
	for (const str of valid) expect(isEmailCore(str, args)).toBe(true)
	for (const str of invalid) expect(isEmailCore(str, args)).toBe(false)
}
describe('isEmail', () => {
	test('should validate strings', () => {
		expect(isEmailCore('foo@bar.com')).toBe(true)
		expect(isEmailCore('foo')).toBe(false)
	})
